...
it is a way for the child compononent to aces a value in a parent component.

- [x] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
1. actions - are payloads of information that sends data from your application to your store. 
2. actions describe that something happened but dont specify how the applications state changes in the response, thats the recuders job
3. store is an object that brings them together

- [x] What is the difference between Application state and Component state? When would be a good time to use one over the other?
 state that is needed by the whole application, or several components. Component state is the state that only a component use the form state, because forms need to handle their own input state but other components don't care to know the state changes that happen while a user is typing in an input.

- [x] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
Its a middleware that lest you call action creator that return a funtion isntead of an action object. that function recievers the store's dispatch

- [x] What is your favorite state management system you've learned and this sprint? Please explain why!
I liked redux store management better it was complicating but once you get the correct dependencies; it becomes to much easier to use
...
- [x] After your API is up and running, you can open chrome and type in `http://localhost:3333/smurfs`. `[]` returned to you. This is an array that your **API** will be using to store our Smurf Data.
